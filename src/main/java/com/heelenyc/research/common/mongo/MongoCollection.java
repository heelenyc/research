package com.heelenyc.research.common.mongo;
...
/**
 * 通用Mongodb层，基于Spring data mongodb框架，主要对MongoTemplate进行了封装，使其所有的操作都对应到一个Collection上，主要原理如下：</p>
 * 1、一个副本集对应一个MongoClient实例；</br> 2、一个DB对应一个MongoTemplate实例；</br> 3、一个Collection对应一个MongoCollection实例； </p>
 * 这样，对于一个MongoCollection实例的所有操作，都对应到初始化时传入的collection上。 </p> 默认属性（可以修改）：</br> <code>
 * ReadPreference = ReadPreference.secondaryPreferred()</br>
 * WriteConcern = WriteConcern.UNACKNOWLEDGED
 * </code> </br> <b>注意：</b>对这两个值的修改将影响到整个JVM里当前Collection的属性，因为MongoCollection是单例的。 </p>
        ...
        if (entityClass.isAnnotationPresent(Document.class)) {
            ...
            if (StringUtils.hasText(doc.collection())) {
                if (doc.collection().equals(collection)) {
                    ...
                    throw new IllegalArgumentException("The entityClass's annotation of 'Document' defined ["
                            + doc.collection() + "] should equal to the collection's name [" + collection + "].");
                }
            }
        }

        // 再检查类名
//        String className = MongoCollectionUtils.getPreferredCollectionName(entityClass);
//        if (className.equals(collection)) {
//            return;
//        } else {
//            throw new IllegalArgumentException("The entityClass's name [" + className
...
//        }
                ...
                if (mongoClient == null) {
                    ...
                    for (String host : hosts) {
                        ...
                        try {
                            ...
                            logger.error(
                                    "replicaSet format should be \"example1.com:27017,example2.com:27017,example3.com:27017\".",
                                    e);
                        }
                    }
    ...
    /**
     * Map the results of an ad-hoc query on the collection for the entity class to a single instance of an object of
     * the specified type.
     * <p/>
    ...
    /**
     * Insert a list of objects into the collection in a single batch write to the database.
     * 
    ...
    /**
     * Save the object to the specified collection. This will perform an insert if the object is not already present,
     * that is an 'upsert'.
     * <p/>
     * The object is converted to the MongoDB native representation using an instance of {@see MongoConverter}. Unless
     * configured otherwise, an instance of SimpleMongoConverter will be used.
     ...
     * String then MongoDB ObjectId will be used to populate that string. Otherwise, the conversion from ObjectId to
     * your property type will be handled by Spring's BeanWrapper class that leverages Spring 3.0's new Type Cobnversion
     * API. See <a href="http://static.springsource.org/spring/docs/3.0.x/reference/validation.html#core-convert">Spring
     * 3 Type Conversion"</a> for more details.
     * 
     * @param objectToSave the object to store in the collection
     */
    ...
    /**
     * Execute a group operation over the entire collection. The group operation entity class should match the 'shape'
     * of the returned object that takes int account the initial document structure as well as any finalize functions.
     * 
     * @param criteria The criteria that restricts the row that are considered for grouping. If not specified all rows
     *            are considered.
     ...
     * entity class should match the 'shape' of the returned object that takes int account the initial document
     * structure as well as any finalize functions.
     * 
     * @param criteria The criteria that restricts the row that are considered for grouping. If not specified all rows
     *            are considered.
     * @param groupBy the conditions under which the group operation will be performed, e.g. keys, initial document,
